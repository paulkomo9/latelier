$(function () {
    ...
    let optModal = $('#calendarentriesModal');
    ...
    /**
     * Render Calendar Entries DataTables
     */
    //let all datatable erros show only on the console/on prod set to ignore
    $.fn.dataTableExt.sErrMode = "console";

    // load all datatable 
    var appointment_status_all = 0;

    var tableCalendarEntries = $('#datatable-calendar-entries')
    ...
    .DataTable({
        ...
        "responsive": true,
        "serverSide": true,
        "language": buildDataTableLanguage('calendar-entries', '#all'),
        "ajax":{
            "url": "/"+locale+"/calendar/list",
            "dataType": "json",
            "type": "POST",
            "data":{ _token:  $('meta[name="csrf-token"]').attr('content'), appointment_status: appointment_status_all}
        },
        "columns": [
            ...
            { "data": "appointment_status", "className": "text-center vertical-center" },
            { "data": "appointment_image"},
            ...
            { "data": "slots_remaining",  "className": "vertical-center" },
        ...
        columnDefs: [
            {
                render: function (data, type, full, meta) {
                    if (data) {
                            ...
                            let imageUrl = full.appointment_image; // already full S3 URL
    ...
    /**
     * Click Button to Create New Calendar Entry
     */
    $('#createNewCalendarEntry').click(function () {

        optModal.modal('show');
     
        //lets clear the error notifications first if any
        clearFormErrors("#calendarentryForm");

        $(".print-error-msg").css('display','none');
        $('#modalCalendarEntriesHeading').html(TRANSLATIONS.addNewCalendarEntry);

        $('#saveBtnCalendarEntry').val("create-calendar-entry");
        $('#saveBtnScheduleName').html(TRANSLATIONS.addCalendarEntry);
         $('#singlePreview').attr("src", "").hide();
        $('#scheduleForm').trigger("reset");

        // Explicitly clear hidden fields
        $('#calendarentryForm').find('input:hidden').val('');
    ...
    /**
     * Store Calendar Entry Code
     */
    $('#saveBtnCalendarEntry').click(function (e) {
        e.preventDefault();
        //
        var $btn = $(this);
        ...
        // serialize form input into array and assign to formCalendarData
        var formCalendarData = $('#calendarentryForm').serializeArray();
        ...
        formCalendarData.forEach(function (field) {
        ...
        $.ajax({
            ...
            error: function (data) {
                ...
                if(data.status == 500){
                    ...
                    $("#calendar-entry-modal-warning").css('display','block');
                    $("#calendar-entry-modal-warning").html("<strong>"+data.responseJSON.error+"</strong>");
                    $spinner.hide();
                    $btn.prop('disabled', false);
    ...
    /**
     * Click Button to Edit Calendar Entry
     */
    $('body').on('click', '.editCalendarEntry', function () {
            var appointment_id = $(this).data('id');
            $.get("/"+locale+"/calendar" +'/' + appointment_id +'/edit', function (data) {

                if(!Object.keys(data).length){
                    //no data found record flagged as deleted;
                    $('#lockedcontentModalTitle').html(TRANSLATIONS.editCalendarEntry);
                    ...
                    if(typeof(data.id) == 'undefined'){
                        ...
                        $(".print-error-msg").css('display','none');
                        $('#modalCalendarEntriesHeading').html(TRANSLATIONS.editCalendarEntry);
                        $('#saveBtnCalendarEntry').val("edit-schedule");
                        $('#saveBtnCalendarEntryName').html(TRANSLATIONS.updateCalendarEntry);
                        ...
                        $('#slots_remaining').val(data.slots - data.slots_taken);
                        ...
                        if (data.appointment_image) {
                            ...
                            let imageUrl = data.appointment_image;
                ...
                let errorMessage = "Failed to fetch calendar entry data.";
    ...
    /**
     * Show Calendar Entry Delete/Deactivate Modal
     */
    $('body').on('click', '.deleteCalendarEntry', function () {
        var appointment_id = $(this).data("id");
            $.get("/"+locale+"/calendar" +'/' + appointment_id +'/edit', function (data) {

                if(!Object.keys(data).length){
                    //no data found record flagged as deleted;
                    $('#lockedcontentModalTitle').html(TRANSLATIONS.deactivateCalendarEntry);
                    $('#lockedcontentModalContent').html(TRANSLATIONS.recordLocked);
                    $('#lockedcontentModal').modal('show');
                    ...
                    if(typeof(data.id) == 'undefined'){
                        ...
                        $('#deleteCalendarEntryModal').modal('show');
                        $(".print-error-msg").css('display','none');
                        $('#deleteCalendarEntryModalTitle').html(TRANSLATIONS.deactivateCalendarEntry);
                        ...
                        $('#del_appointment_id').val(data.id);
                        $('#deleteCalendarEntryModalContent').html(content);
    ...
    /**
     * Delete Deactivate Calendar Entry
     */
    $('#deleteBtnCalendarEntry').click(function (e) {
        e.preventDefault();
        //
        var $btn = $(this);
        var $spinner = $btn.find('.fa-spinner');
        ...
        var appointment_id = $('#del_appointment_id').val();
        //you selected the OK button
        $.ajax({
            type: "DELETE",
            url:"/"+locale+"/calendar/"+appointment_id,
            success: function (data) {

                if(typeof(data.success) == 'undefined'){
                    //the session has expired
                    $('#deleteCalendarEntryModal').modal('hide');
                    $('#sessionexpiredModalTitle').html("Session Expired");
                    $('#sessionexpiredModalContent').html("Your session has expired. To continue accessing our platform, please click the Login button below to log in again.");
                    $('#sessionexpiredModal').modal('show');
